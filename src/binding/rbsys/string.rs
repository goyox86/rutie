...
pub fn new_from_bytes(bytes: &[u8], enc: Value) -> Value {
    ...
    unsafe { string::rb_enc_str_new(bts, len, encoding::rb_to_encoding(enc.into())).into() }
}
...
pub fn value_to_string(value: Value) -> String {
    unsafe {
        let str = string::rb_string_value_cstr(&value as *const _ as *mut u64);
...
pub fn value_to_bytes_unchecked<'a>(value: Value) -> &'a [u8] {
    unsafe {
        let str = string::rb_string_value_ptr(&value as *const _ as *mut u64) as *const u8;
